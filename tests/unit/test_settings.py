...
def test_auto_detects_openrouter(no_keys, monkeypatch):
    ...
    assert s.resolve_model() == "meta-llama/llama-3.1-70b-instruct"
...
def test_nvidia_model_keeps_valid_id(no_keys, monkeypatch):
    reset_settings()
    monkeypatch.setenv("AGENT_LLM_PROVIDER", "nvidia")
    monkeypatch.setenv("AGENT_LLM_MODEL", "meta/llama-3.1-70b-instruct")
    s = get_settings()
    assert s.resolve_provider() == "nvidia"
    assert s.resolve_model() == "meta/llama-3.1-70b-instruct"
...
def test_nvidia_provider_direct():
    ...
    provider = NvidiaProvider(
        api_key="fake", model="meta/llama-3.1-70b-instruct"
    )
